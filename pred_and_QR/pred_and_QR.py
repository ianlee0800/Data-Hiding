...
#X1:預估式嵌入法，嵌入隨機位元
img_p = generate_perdict_image(origImg, weight) #生成預測影像
#a.差值直方圖處理
diffA = two_array2D_add_or_subtract(origImg, img_p, -1) #兩個二維方陣差值
payload_diff = calculate_payload(diffA, EL) #由EL決定最大嵌入量
bpp_diff = round(payload_diff/origImg.size, 2) 
diffId, diffNum = generate_different_histogram_without_frame(diffA, list(range(-range_x,range_x+1)), [0]*(range_x*2+1))#差值直方圖
print(diffNum) #差值直方圖數值
#b. 直方圖左右偏移，移出嵌入空間
...
#差值直方圖與預測影像還原成已嵌入影像
img_diff = two_array2D_add_or_subtract(img_p, diffA_e, 1) #得到已嵌入影像
...
#二維碼：模塊形狀調整，嵌入加權值與EL
data_p = weight.copy()
data_p.append(EL) #嵌入加權值與EL
bin_weight = int_transfer_binary_single_intlist(data_p, "{0:08b}") #轉換為二進制
#二維碼前處理
bits, simQrcode = simplified_qrcode(QRCImg)#簡化二維碼得知版本
loc = find_locaion(QRCImg, bits)#尋找二維碼模塊位置
#模塊形狀調整
if method == "h":#水平嵌入法
    payload_qr = calculate_embedded_bits(simQrcode, 1)#得到二維碼有效負載
    message = repeat_int_array(bin_weight, payload_qr)#隱藏資訊重複嵌入
    QRCImg_m = horizontal_embedding(QRCImg, simQrcode, loc, message, k)#模塊形狀調整
elif method == "v":#垂直嵌入法
    payload_qr = calculate_embedded_bits(simQrcode, 2)#得到二維碼有效負載
    message = repeat_int_array(bin_weight, payload_qr)#隱藏資訊重複嵌入
    QRCImg_m = vertical_embedding(QRCImg, simQrcode, loc, message, k)#模況形狀調整

#X2：DE嵌入法，嵌入二進制二維碼
binQRC_m = array2D_transfer_to_array1D(QRCImg_m)#二維碼轉換為二進制
#使用DE將二維碼嵌入進影像中
...
#X3：直方圖平移嵌入法，嵌入可嵌入地圖
bin_map = int_transfer_binary_single_intlist(locationmap, "{0:09b}")#可嵌入地圖轉換為二進制
...
#DE解碼，取出已嵌入二維碼二進制
exImg_diff, exInf_qr = decode_different_expansion(markedImg)
flag_img = same_array2D(exImg_diff, img_diff) and calculate_psnr(exImg_diff, img_diff) == 0 and calculate_ssim(exImg_diff, img_diff)
...
exImg_p = generate_perdict_image(origImg, exWeight)
...
plt.ylim(0, diffNum[range_x+1]*1.3)
plt.savefig("./pred_and_QR/outcome/histogram/%s/difference/de_%s_diffembed.%s"%(imgName, imgName, filetype))
plt.close()
#解碼已嵌入差值直方圖得到隱藏資訊
...
plt.ylim(0, diffNum[range_x+1]*1.3)
plt.savefig("./pred_and_QR/outcome/histogram/%s/difference/de_%s_diffshift.%s"%(imgName, imgName, filetype)) 
plt.close()  
#將已平移過的直方圖復原
...
plt.ylim(0, diffNum[range_x+1]*1.3)  
plt.savefig("./pred_and_QR/outcome/histogram/%s/difference/de_%s_diff.%s"%(imgName, imgName, filetype))
plt.close()
# plt.show()
