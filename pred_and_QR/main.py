...
def main():
    ...
    try:
        ...
        if pee_stages:
            print("Debug: PEE stages generated successfully")
            for i, stage in enumerate(pee_stages):
                ...
                # 保存圖像
                save_image(to_numpy(stage['image']), f"./pred_and_QR/outcome/image/{imgName}/{imgName}_X1_rotation_{i}.{filetype}")
                
                # 保存直方圖
                save_histogram(to_numpy(stage['image']), 
                            f"./pred_and_QR/outcome/histogram/{imgName}/X1_rotation_{i}_histogram.png", 
                            f"Histogram of PEE Rotation {i}")
                
                # 保存差異直方圖
                if i > 0:
                    prev_img = to_numpy(pee_stages[i-1]['image'])
                else:
                    prev_img = origImg
                
                diff_before = to_numpy(stage['image']) - prev_img
                ...
                diff_after = to_numpy(stage['image']) - prev_img
            ...
            try:
                ...
                print(f"X2 (Histogram Shifting): Peak={peak}, Payload={payload_hs}, BPP={final_bpp:.4f}")
                print(f"Total Payload={total_payload + payload_hs}, Total BPP={final_bpp:.4f}")
            ...
            print("Warning: No PEE stages were generated. Check the pee_process_with_rotation_cuda function.")
