...
def plot_interval_statistics(df, imgName, method, prediction_method, output_dir=None):
    """
    Create line plots of interval statistics data
    ...
    output_dir : str, optional
        Output directory, default is "./Prediction_Error_Embedding/outcome/plots/{imgName}"
        
    Returns:
    --------
    tuple
        (fig_psnr, fig_ssim) PSNR and SSIM charts, returns (fig_psnr, fig_ssim, fig_hist) if histogram correlation data is available
    """
    ...
    for i, row in enumerate(df_sorted.itertuples()):
        if i % 3 == 0 or i == len(df_sorted) - 1:  # Only label some points to avoid crowding
    ...
    plt.close()  # Ensure chart is closed to release resources
    
    # Create BPP-SSIM line chart
    ...
    # If histogram correlation data is available, also create a line chart for it
