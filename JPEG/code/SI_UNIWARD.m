function [ S_STRUCT , C_STRUCT , pChange ] = SI_UNIWARD(precover , C_STRUCT , payload)
...
precover_PADDED = padarray(precover, [padSize padSize], 'symmetric'); % pad image

R_PC = cell(size(F));
for i=1:numel(F)
    R_PC{i} = imfilter(precover_PADDED, F{i});
end
